...
*********************************************************************/
double chassis::turn(double angle)
{
    const double T = 0.001;     //采样间隔
    const double radius = tread / 2;

    double kp = 3.3880882481012677 * radius, ki = 0.00018496649234671578 * radius, kd = 8.662355267155778e-05 * radius;   //PID的三个常量，明天一定改好
    ...
    while (fabs(error_angle) > 0.05){
        ...
        //更新位置
        chassis_angle = -propotion * (LeftMotorGroup.position(rev) - init_pos) / radius;
...
*********************************************************************/
double chassis::move(double distance)
{

    double kp = 2.3880882481012677, ki = 0.00018496649234671578, kd = 8.662355267155778e-05;   //PID的三个常量，明天一定改好
...
void chassis::go_to(double x, double y, int R, int mid_chassis) 
{
    ...
    double actual_angle = turn(angle) + toward;    //调旋转函数
    ...
    co.x += actual_distance * cos(toward);
    co.y += actual_distance * sin(toward);
    if (!mid_chassis) {
